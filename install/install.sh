#!/bin/bash
# ============================================================
# U-Claw One-Line Installer (Mac/Linux)
# Usage: curl -fsSL https://u-claw.org/install.sh | bash
#    or: bash install.sh
# ============================================================
...
cat << 'BANNER'
  ╔══════════════════════════════════════════╗
  ║  🦞 U-Claw Installer                     ║
  ║  One-line install for AI assistant       ║
  ╚══════════════════════════════════════════╝
BANNER
echo -e "${NC}"
...
if [ "$OS" = "Darwin" ]; then
    ...
    echo -e "  ${YELLOW}For Windows, use PowerShell: irm https://u-claw.org/install.ps1 | iex${NC}"
    exit 1
fi
...
if [ -d "$UCLAW_DIR/core/node_modules/openclaw" ]; then
    echo -e "  ${YELLOW}Existing installation detected: $UCLAW_DIR${NC}"
    if [ -t 0 ]; then
        ...
        echo -e "  ${CYAN}Pipe mode, auto-overwriting${NC}"
    fi
    echo ""
fi
...
if [ "$USE_SYSTEM_NODE" = "false" ]; then
    if [ -f "$NODE_INSTALL_DIR/bin/node" ]; then
        echo -e "  ${GREEN}✓${NC} Node.js already exists, skipping download"
        INSTALL_NODE="$NODE_INSTALL_DIR/bin/node"
        INSTALL_NPM="$NODE_INSTALL_DIR/bin/npm"
    else
        echo -e "  ${CYAN}↓${NC} Downloading Node.js $NODE_VERSION ($PLATFORM) from China mirror..."
        ...
        if command -v curl >/dev/null 2>&1; then
            ...
            echo -e "  ${RED}✗ curl or wget not found, please install one${NC}"
            exit 1
        fi
...
PKGJSON
    ...
    echo -e "  ${CYAN}↓${NC} Installing from China mirror..."
...
# Step 4: QQ 插件（非致命）
# ============================================================
echo -e "  ${BOLD}[3/7] Installing QQ Plugin ...${NC}"
...
if [ -d "$CORE_DIR/node_modules/@sliverp/qqbot" ]; then
    ...
    "$INSTALL_NODE" "$INSTALL_NPM" install @sliverp/qqbot@latest --prefix "$CORE_DIR" --registry="$MIRROR" 2>/dev/null || {
        echo -e "  ${YELLOW}⚠${NC}  QQ plugin install failed (does not affect main features)"
    }
    if [ -d "$CORE_DIR/node_modules/@sliverp/qqbot" ]; then
